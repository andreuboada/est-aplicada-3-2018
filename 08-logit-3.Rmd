...
```{r, out.width='30%', echo=FALSE, eval=FALSE}
df <- tibble(x1=c(1,1,2,3),x2=c(0.5,1.5,1,1),y=c(0,0,1,0))
g <- ggplot(df, aes(x=x1, y=x2, color=as.factor(y), shape=as.factor(y))) + 
  geom_point(size=3) + 
  theme_bw() + guides(color=FALSE, shape=FALSE) +
  xlab(expression(~x[{1}])) + ylab(expression(~x[{2}]))
ggsave(filename = "~/Desktop/grafica.pdf", plot = g, device = "pdf", width = 3, height = 3)
```
