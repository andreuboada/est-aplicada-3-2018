...
Para esto usaremos la librería `dplyr` que contiene funciones que facilitan la implementación de la  estrategia.
```
...
```{r echo=FALSE, fig.align='center', dpi=350}
knitr::include_graphics("figuras/visual-variables.png")
```
...
factores_inglehart_etiq <- etiquetar(factores_inglehart, "survival_selfexpression",
                                     "traditional_secular", etiq = "country", size = 0.8)
```
...
ggplot(data = factores_inglehart_etiq,
  ...
  geom_point(size = 1) +
  geom_text(aes(x = a, y = b, label = country), size = 1.7) +
...
```{r echo=FALSE, fig.align='center', dpi=200}
knitr::include_graphics("figuras/billboard.png")
```
