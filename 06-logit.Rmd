...
Podemos ajustar varios modelos para mostrar que hay incertidumbre en el ajuste del modelo. En el rango de los datos, la línea sólida muestra el mejor ajuste para una regresión logística, y las líneas de color gris claro muestran la incertidumbre en el ajuste.

```{r}
M <- 500
N <- nrow(datos_2)

glm.1 <- glm(vote~income, data = datos_2)
b0_mean <- summary(glm.1)$coefficients[1,1]
b0_sd <- summary(glm.1)$coefficients[1,2]
b1_mean <- summary(glm.1)$coefficients[2,1]
b1_sd <- summary(glm.1)$coefficients[2,2]

sim_params <- function(i){
  b0_sim <- rnorm(1, b0_mean, b0_sd)
  b1_sim <- rnorm(1, b1_mean, b1_sd)
  tibble(sim=i,b0_sim=b0_sim,b1_sim=b1_sim)
}

modelos <- map_df(1:M, sim_params)

modelos <- 1:M %>%
  map(~glm(vote~income, data = sample_n(tbl = datos_2, size = 10, replace = T))) %>%
  map(summary) %>% 
  map("coefficients") %>%
  map_df(function(x){data_frame(intercept=x[1,1], income=x[2,1])})

g <- ggplot(datos_2, aes(x=vote)) #+
#  geom_jitter(width = 0.1, height = 0.08, size = 0.1)

for(i in 1:M){
  g <- g + stat_function(fun = function(x){invlogit(modelos$b0_sim[i] + modelos$b1_sim[i]*x)})
}

g + stat_function(fun = function(x){invlogit(b0 + b1*x)}, xlim = c(0,6), size = 1.5)
g
```
-->
